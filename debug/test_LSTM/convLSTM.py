...
class CLSTM_cell(nn.Module):
    ...
    def __init__(self, shape, input_chans, filter_size, num_features):
        ...
        self.num_features = num_features    # num_features也就是卷积层的out_channels数
...
class CLSTM(nn.Module):
    ...
    def __init__(self, shape, input_chans, filter_size, num_features,num_layers):
        ...
        cell_list=[]
        cell_list.append(CLSTM_cell(self.shape, self.input_chans, self.filter_size, self.num_features).cuda())# the first one has a different number of input channels
    ...
    def forward(self, input, hidden_state):
        ...
        for idlayer in range(self.num_layers):#loop for every layer
            ...
            hidden_c=hidden_state[idlayer]#hidden and c are images with several channels. zcr: 这里的hidden_c包括(hidden,c), 这个hidden_c其实就是特定层初始化的(hidden, c)
            all_output = []
            output_inner = []
            for t in range(seq_len):#loop for every step
                ...
                output_inner.append(hidden_c[0])    # hidden_c是一个(next_hidden, next_c)的元组
                # print('&' * 10, hidden_c[0].size()) # torch.Size([16, 10, 25, 25]
...
if __name__ == '__main__':
    ...
    # print('hidden states: ', hidden_state)
    print('hidden states length: ',len(hidden_state))   # 2
    # for i in range(len(hidden_state)):
    #     print(i, len(hidden_state[i]))  # 都是2
    # print('hidden_h shape ',hidden_state[0][0].size())  # torch.Size([16, 10, 25, 25])
    # print('hidden_h shape ',hidden_state[0][1].size())  # torch.Size([16, 10, 25, 25])
    # print('hidden_h shape ',hidden_state[1][0].size())  # torch.Size([16, 10, 25, 25])
    # print('hidden_h shape ',hidden_state[1][1].size())  # torch.Size([16, 10, 25, 25])
    out=conv_lstm(input,hidden_state)
    print('out shape',out[1].size())    # ([4, 16, 10, 25, 25])
    print('len hidden ', len(out[0]))   # 2
    print('next hidden',out[0][0][0].size())    # torch.Size([16, 10, 25, 25])
